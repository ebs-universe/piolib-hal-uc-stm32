...
void id_init(void){
    #if uC_ID_TRIMFUNC == 1
        const uint32_t* uid_base_word = (const uint32_t*) UID_BASE;
        uint8_t uid_bytes[12];
        uint64_t lot_number = 0;

        for (int j = 0; j < 3; j++) {
            uint32_t word = *uid_base_word++;
            uid_bytes[j * 4 + 0] = (word >> 0) & 0xFF;
            uid_bytes[j * 4 + 1] = (word >> 8) & 0xFF;
            uid_bytes[j * 4 + 2] = (word >> 16) & 0xFF;
            uid_bytes[j * 4 + 3] = (word >> 24) & 0xFF;
        }
        
        for (HAL_BASE_t i = 0; i < 7; i ++) {
            uint8_t current_value;
            uint8_t current_char = uid_bytes[5 + i];
        ...
        device_id.bytes[5] = uid_bytes[4];

        int16_t x_coord = *((int16_t*)(&uid_bytes[0]));
        int16_t y_coord = *((int16_t*)(&uid_bytes[2]));
